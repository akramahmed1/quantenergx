    ...
    async def connect(self):
        ...
        try:
            self.pool = await asyncpg.create_pool(
                ...
                command_timeout=60,
                server_settings={"jit": "off", "application_name": "quantenergx-backend"},
            )
            ...
            logger.info(
                f"Database connected successfully with pool size {settings.db_min_connections}-{settings.db_max_connections}"
            )
