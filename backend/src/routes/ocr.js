...
router.get('/status', (req, res) => {
  res.json({
    ...
    status: 'online',
    supported_languages: [
      'eng', 'ara', 'fas', 'chi_sim', 'rus', 'fra', 'spa', 'urd'
    ],
...
router.post('/process', 
  ...
  [
    ...
    body('extractFields').optional().isBoolean()
      .withMessage('extractFields must be a boolean'),
    body('detectStamps').optional().isBoolean()
      .withMessage('detectStamps must be a boolean'),
...
router.post('/extract-fields/:documentId',
  authenticateToken,
  [
    body('fieldDefinitions').isArray()
      .withMessage('fieldDefinitions must be an array'),
    body('fieldDefinitions.*.name').notEmpty()
      .withMessage('Field name is required'),
    body('fieldDefinitions.*.type').isIn(['text', 'number', 'date', 'currency'])
...
router.post('/review/:documentId',
  authenticateToken,
  [
    body('corrections').isObject()
      .withMessage('corrections must be an object'),
    body('reviewStatus').isIn(['approved', 'rejected', 'needs_revision'])
