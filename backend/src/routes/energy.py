...
async def get_energy_prices(
    ...
    while current_date <= date_to:
        ...
        price_data = EnergyPriceResponse(
            market=market.upper(),
            date=current_date,
            price_per_mwh=round(base_price * regional_multiplier * (1 + random.uniform(-0.1, 0.1)), 2),
            region=region,
    ...
    response_data = EnergyPriceListResponse(
        prices=energy_prices,
        total=len(energy_prices)
    )
    ...
    await redis_client.set(
        ...
        ttl=settings.energy_price_cache_ttl
    )
...
async def get_current_energy_price(
    market: str,
    region: str = Query("US_WEST", description="Market region")
) -> EnergyPriceResponse:
    ...
    current_price = EnergyPriceResponse(
        market=market.upper(),
        date=date.today(),
        price_per_mwh=round(base_price * regional_multiplier * (1 + random.uniform(-0.05, 0.05)), 2),
        region=region,
    ...
    await redis_client.set(
        cache_key,
        current_price.model_dump(mode='json'),
        ttl=30
    )
...
async def clear_energy_price_cache(market: str, region: str = Query("US_WEST")):
    ...
    return {
        "message": f"Cache cleared for {market} in {region}",
