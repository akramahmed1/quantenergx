...
router.post(
  ...
  [
    body('channel')
      .isIn(['email', 'whatsapp', 'telegram', 'sms'])
...
router.post(
  '/test',
  authenticateToken,
  [body('channel').isIn(['email', 'whatsapp', 'telegram', 'sms']), body('recipient').notEmpty()],
  async (req, res) => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
...
Timestamp: ${new Date().toISOString()}`;
      ...
      const result = await notificationService.sendNotification(channel, recipient, testMessage, {
...
router.post('/ocr/document-processed', authenticateToken, async (req, res) => {
  try {
...
router.post('/ocr/review-required', authenticateToken, async (req, res) => {
  try {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const notifications = await notificationService.notifyTradeExecuted(trade, userPreferences);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const notifications = await notificationService.notifyOrderPlaced(order, userPreferences);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const notifications = await notificationService.notifyMarginCall(marginData, userPreferences);
