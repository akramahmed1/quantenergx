...
class ExternalAPIException(QuantEnergxException):
    ...
    def __init__(
        self, message: str = "External API error", details: Optional[Dict[str, Any]] = None
    ):
...
async def quantenergx_exception_handler(
    request: Request, exc: QuantEnergxException
) -> JSONResponse:
...
async def general_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    return create_error_response(
        request=request, status_code=500, message=message, log_level="error"
    )
