...
class MarketDataService {
  ...
  _generateSimulatedData(commodity, symbol, timeframe) {
    ...
    for (let i = 0; i < dataPoints; i++) {
      ...
      prices.push({
        ...
        high: currentPrice * (1.00 + Math.random() * 0.02),
  ...
  _aggregateAnalytics(analyticsArray) {
    ...
    return {
      ...
      price: {
        ...
        average: analyticsArray.reduce((sum, a) => sum + a.price.average, 0) / analyticsArray.length,
        min: Math.min(...analyticsArray.map(a => a.price.min)),
        max: Math.max(...analyticsArray.map(a => a.price.max)),
        changePercent: analyticsArray.reduce((sum, a) => sum + a.price.changePercent, 0) / analyticsArray.length
      },
      volume: {
        total: analyticsArray.reduce((sum, a) => sum + a.volume.total, 0),
        average: analyticsArray.reduce((sum, a) => sum + a.volume.average, 0) / analyticsArray.length
      },
  ...
  _identifyTrends(analytics) {
    ...
    return {
      ...
      volatilityRegime: analytics.volatility > 0.3 ? 'high' : analytics.volatility > 0.15 ? 'medium' : 'low'
    };
  }
  ...
  async _calculateCorrelations(commodity, period) {
    ...
    for (const otherCommodity of correlationCommodities.slice(0, 3)) { // Limit for demo
      try {
  ...
  _generateMarketOverview(marketAnalytics) {
    ...
    const avgVolatility = marketAnalytics.reduce((sum, ma) => sum + ma.analytics.volatility, 0) / marketAnalytics.length;
    
    return {
      totalMarketVolume: totalVolume,
      averageVolatility: avgVolatility,
      bullishCommodities: marketAnalytics.filter(ma => ma.trends.direction === 'bullish').map(ma => ma.commodity),
      bearishCommodities: marketAnalytics.filter(ma => ma.trends.direction === 'bearish').map(ma => ma.commodity),
  ...
  _generateMarketRecommendations(marketAnalytics) {
    ...
    marketAnalytics.forEach(ma => {
      if (ma.trends.strength === 'strong') {
        recommendations.push(`Strong ${ma.trends.direction} trend in ${ma.commodity} - consider momentum strategies`);
      }
      
      if (ma.volatilityMetrics.regime === 'high') {
        recommendations.push(`High volatility in ${ma.commodity} - implement risk management strategies`);
      }
    });
