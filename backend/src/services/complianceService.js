...
class ComplianceService {
    ...
    this.regulatoryFrameworks = {
      ...
      ME: {
        islamic_finance: {
          ...
          description: 'Sharia-compliant trading requirements',
        },
        local_regulatory: {
          name: 'Local Regulatory',
          description: 'Regional regulatory frameworks',
        },
      },
    };
  ...
  async performComplianceCheck(transactionData, region = 'US') {
    ...
    if (missingFields.length > 0) {
      throw new Error(
        `Compliance check failed: Missing required fields: ${missingFields.join(', ')}`
      );
    }
  ...
  async _checkTransactionReporting(transactionData, region) {
    ...
    const requiresReporting =
      volume >= reportingThresholds.volumeThreshold || value >= reportingThresholds.valueThreshold;
