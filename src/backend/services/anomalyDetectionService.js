...
class AnomalyDetectionService {
  ...
  async detectAnomalies(data, commodities = ['oil', 'gas', 'renewable'], methods = ['all']) {
    try {
      ...
      detectionResult.anomalies_detected = this.consolidateAnomalies(
        detectionResult.anomalies_detected
      );
      detectionResult.summary = this.generateSummary(detectionResult.anomalies_detected);
      detectionResult.recommendations = this.generateRecommendations(
        detectionResult.anomalies_detected
      );
      detectionResult.confidence_scores = this.calculateConfidenceScores(
        detectionResult.anomalies_detected
      );
  ...
  async detectCorrelationAnomalies(data, commodities) {
    ...
    for (let i = 0; i < commodities.length; i++) {
      for (let j = i + 1; j < commodities.length; j++) {
        ...
        const corrAnomalies = this.detectPairwiseCorrelationAnomalies(
          data1,
          data2,
          commodity1,
          commodity2
  ...
  detectIQRAnomalies(data, commodity) {
    ...
    data.forEach(point => {
      if (point.price < lowerBound || point.price > upperBound) {
        const severity =
          point.price < q1 - 3 * iqr || point.price > q3 + 3 * iqr ? 'SEVERE' : 'MODERATE';
  ...
  detectSeasonalAnomalies(data, commodity) {
    ...
    data.forEach(point => {
      ...
      if (deviation > 0.15) {
        // 15% deviation from seasonal pattern
        anomalies.push(
          this.createAnomaly({
  ...
  detectVolumePriceDivergence(data, commodity) {
    ...
    for (let i = window; i < data.length; i++) {
      ...
      const volumeChange =
        ((data[i].volume || 1000) - (data[i - window].volume || 1000)) /
        (data[i - window].volume || 1000);
  ...
  generateSummary(anomalies) {
    ...
    if (Object.keys(commodityCounts).length > 0) {
      summary.most_affected_commodity = Object.keys(commodityCounts).reduce((a, b) =>
        commodityCounts[a] > commodityCounts[b] ? a : b
      );
    }
  ...
  calculateConfidenceScores(anomalies) {
    ...
    Object.keys(methodGroups).forEach(method => {
      scores.by_method[method] =
        methodGroups[method].reduce((sum, c) => sum + c, 0) / methodGroups[method].length;
    });
    ...
    Object.keys(typeGroups).forEach(type => {
      scores.by_type[type] =
        typeGroups[type].reduce((sum, c) => sum + c, 0) / typeGroups[type].length;
    });
  ...
  getRecentAnomalies(hours = 24) {
    ...
    const recentAnalyses = Array.from(this.anomalies.values()).filter(
  ...
  updateBaselines(commodity, data) {
    ...
    const stats = {
      ...
      std: this.calculateStandardDeviation(
        data.map(d => d.price),
        this.calculateMean(data.map(d => d.price))
      ),
